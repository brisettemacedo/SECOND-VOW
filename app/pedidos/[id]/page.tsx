...
export default async function OrderDetail({ params, searchParams }: { params: Promise<{ id: string }>; searchParams?: Promise<{ payment?: string }> }) {
  ...
  const { data: order } = await supabase.from("orders").select("*,dresses(id,model,brands(name)),claims(*),shipments(*)").eq("id", id).or(`buyer_id.eq.${user.id},seller_id.eq.${user.id}`).maybeSingle();
  if (!order) notFound();
