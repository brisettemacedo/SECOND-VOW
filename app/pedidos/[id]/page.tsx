...
export default async function OrderDetail({ params, searchParams }: { params: Promise<{ id: string }>; searchParams?: Promise<{ payment?: string }> }) {
  ...
  return <main className="page narrow">
    ...
    <section className="panel"><p>Vestido: ${Number(order.subtotal_mxn).toLocaleString("es-MX")} MXN</p>{order.shipping_quote_set_at ? <p>Envío: ${Number(order.shipping_mxn).toLocaleString("es-MX")} MXN{order.shipping_carrier_declared ? ` con ${carrierName(order.shipping_carrier_declared)}` : ""}</p> : <p className="muted">Envío pendiente de cotización.</p>}<p>Estado: <span className="badge">{ORDER_STATUS[order.status] || order.status}</span></p>{order.tracking_number && <p>Rastreo: {carrierName(order.carrier)} | {order.tracking_number}</p>}</section>
