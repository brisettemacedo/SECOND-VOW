...
export default async function AdminPublicationDetail({ params }: { params: { id: string } }) {
  ...
  const { data: dress, error } = await supabase
    ...
    .select(`
      *,
      brands(name),
      brand_suggestions(suggested_name,status),
      dress_photos(id,storage_path,is_primary,position,classification,created_at),
      dress_characteristics(characteristic_id,characteristics(label)),
      dress_declarations(authenticity_declared,photos_correspond_declared,right_to_sell_declared,information_true_declared,terms_version,declared_at),
      dress_moderation_history(id,action,status_from,status_to,comments,admin_id,created_at)
    `)
    .eq("id", params.id)
    .maybeSingle();

  if (error || !dress) notFound();

  const photos = [...(dress.dress_photos ?? [])].sort((a: any, b: any) => (b.is_primary ? 1 : 0) - (a.is_primary ? 1 : 0) || a.position - b.position);
  ...
  const chars = (dress.dress_characteristics ?? []).map((item: any) => {
    const relation = Array.isArray(item.characteristics) ? item.characteristics[0] : item.characteristics;
    return relation?.label;
  }).filter(Boolean);
  const declarations = Array.isArray(dress.dress_declarations) ? dress.dress_declarations[0] : dress.dress_declarations;
  const history = [...(dress.dress_moderation_history ?? [])].sort((a: any, b: any) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime());

  const rows: [string, any][] = [
    ["Marca", brand], ["Modelo", dress.model || "—"], ["Colección", dress.collection || "—"], ["Año aproximado", dress.year_approx || "—"],
  ...
  return (
    <main className="page admin-publication-detail">
      <div className="title-row">
        <div><span className="badge">{dress.status}</span><h1>{brand} {dress.model ?? ""}</h1></div>
        ...
        <div className="admin-photo-grid">
          {photos.map((photo: any) => (
            <figure key={photo.id}>
              ...
              <figcaption>{photo.is_primary ? "Principal" : photo.classification || `Posición ${photo.position + 1}`}</figcaption>
            </figure>
          ))}
          {!photos.length && <p>No hay fotografías.</p>}
        </div>
      </section>
      ...
      <section className="panel">
        ...
        {declarations ? <ul className="admin-declarations">
          ...
          <li>Declarado: {new Date(declarations.declared_at).toLocaleString("es-MX")}</li>
        </ul> : <p>No hay declaraciones registradas.</p>}
      </section>
