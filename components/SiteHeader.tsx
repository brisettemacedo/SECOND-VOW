...
export default async function SiteHeader() {
  ...
  let actionableOffers = 0;
  let actionableOrders = 0;

  if (user) {
    const [{ data: profile }, unreadRes, offersRes, ordersRes] = await Promise.all([
      supabase.from("profiles").select("role").eq("id", user.id).maybeSingle(),
      supabase.from("messages").select("id", { count: "exact", head: true }).is("read_at", null).neq("sender_id", user.id),
      supabase.from("offers").select("id", { count: "exact", head: true }).eq("status", "pending").neq("created_by", user.id).or(`buyer_id.eq.${user.id},seller_id.eq.${user.id}`),
    ...
    actionableOffers = offersRes.count ?? 0;
  ...
  return (
    <header className="site-header">
      ...
      <nav>
        ...
        {user && <Link className="nav-with-count" href="/ofertas">Ofertas<CountBadge count={actionableOffers} /></Link>}
