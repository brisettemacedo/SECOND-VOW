...
import { ESTADOS_MX } from "@/lib/catalogs";
...
export default function AccountForm({
  ...
  async function saveProfile(e: React.FormEvent) {
    ...
    const { error } = await supabase
      .from("profiles")
      .update({ full_name: fullName, city, state })
      .eq("id", user.id);

    setSavingProfile(false);
    setProfileMsg(
      error
        ? { type: "error", text: "No se pudo guardar. Intenta de nuevo." }
        : { type: "success", text: "Perfil actualizado." }
    );
  }
  ...
  async function changePassword(e: React.FormEvent) {
    ...
    setPasswordMsg(
      error
        ? { type: "error", text: "No se pudo cambiar la contraseña." }
        : { type: "success", text: "Contraseña actualizada." }
    );
  }
