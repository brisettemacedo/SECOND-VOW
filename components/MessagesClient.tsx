...
export default function MessagesClient({initial,userId,initialActive}:{initial:Conv[];userId:string;initialActive?:string}){
 ...
 async function offerAction(id:string,action:"accept"|"decline"){
   ...
   const res=action==="accept"
     ...
     :await supabase.rpc("decline_offer",{p_offer_id:id});
   setBusy(false);
 ...
 const canOffer=activeConv?.seller_id===userId && activeConv?.dresses?.precio_venta_mxn;
 ...
 const hasExpiredOfferForBuyer=activeConv?.buyer_id===userId && (activeConv?.offers??[]).some((o:any)=>
   (o.status==="expired") || (o.status==="pending" && new Date(o.expires_at).getTime()<=Date.now())
 );
 ...
 return <div className="messaging">
  ...
  <section className="chat panel">{activeConv?<>
    ...
    {activeConv?.seller_id===userId&&<div className="alert-info">Tu oferta debe incluir el costo del envío — es lo único que va a pagar la compradora, sin sorpresas después. SECOND VOW retiene 18% sobre vestido + envío (ya incluye Stripe); el resto se transfiere a tu cuenta una vez que la compradora confirme la recepción. Recomendamos usar un envío con guía y seguro — para compras de $10,000 MXN o más es obligatorio.</div>}
    <div className="messages">{timeline.map((item:any)=>{if(item.kind==="message"){const m=item.data;return <div key={`m-${m.id}`} className={m.sender_id===userId?"bubble mine":"bubble"}>{m.body}<small>{new Date(m.created_at).toLocaleString("es-MX")}</small></div>}
      if(item.kind==="offer"){
        ...
        const canRespond=isLive&&o.created_by!==userId;
        return <div key={`o-${o.id}`} className="commerce-event">
          ...
          {canRespond&&<div className="commerce-actions">
            <button className="btn btn-primary" disabled={busy} onClick={()=>offerAction(o.id,"accept")}>Aceptar y pagar</button>
