...
export default function MessagesClient({initial,userId,initialActive}:{initial:Conv[];userId:string;initialActive?:string}){
 ...
 return <div className="messaging">
  ...
  <section className="chat panel">{activeConv?<>
    ...
    {detailsLoading?<div className="chat-loading">Cargando conversación…</div>:<div className="messages">{timeline.map((item:any)=>{if(item.kind==="message"){const m=item.data;return <div key={`m-${m.id}`} className={m.sender_id===userId?"bubble mine":"bubble"}>{m.body}<small>{new Date(m.created_at).toLocaleString("es-MX")}</small></div>}
      if(item.kind==="offer"){
        const o=item.data;
