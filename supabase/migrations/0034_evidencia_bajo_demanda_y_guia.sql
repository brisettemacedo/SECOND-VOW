-- La evidencia se conserva fuera de la plataforma durante una operación normal
-- y se carga únicamente si existe una controversia. La guía, en cambio, sí se
-- registra y comparte para habilitar el seguimiento.
-- Retira todas las firmas anteriores para impedir que una llamada directa a
-- PostgREST eluda los requisitos vigentes mediante un overload antiguo.
drop function if exists public.mark_order_shipped(uuid,text,text);
drop function if exists public.mark_order_shipped(uuid,text,text,boolean,boolean);
drop function if exists public.mark_order_shipped(uuid,text,text,boolean,boolean,boolean);
...
begin
  ...
  insert into public.order_events(order_id,actor_id,event_type,metadata) values(o.id,(select auth.uid()),'shipped',jsonb_build_object('carrier',btrim(p_carrier),'tracking_number',btrim(p_tracking_number),'insured',true,'signature',true,'id_delivery',true,'evidence_retained',true));
  insert into public.notifications(user_id,order_id,dress_id,kind,title,body) values(o.buyer_id,o.id,o.dress_id,'shipment_registered','Tu vestido fue enviado','Guía '||btrim(p_tracking_number)||' con '||btrim(p_carrier)||'. Ya puedes seguir el envío desde tu pedido. La entrega requiere firma e identificación oficial.');
end $$;
